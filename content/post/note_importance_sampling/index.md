...
Notice that the algorithm above is not valid when $\boldsymbol{A}\_t$ is not positive definite, since it would not be possible to implement step 4(a). In this case \cite{JungbackerKoopman2007} show that, as long as $\boldsymbol{A}\_t$ is invertible, the sample $\tilde{\vtheta}_t^{(i)}$ is generated by the following simulation smoothing recursions:
