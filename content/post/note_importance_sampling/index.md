...
3. Draw $\boldsymbol{\theta}\_t^+$ from $g(\boldsymbol{\theta}\_t)$, i.e. the (approximate) linear Gaussian model for $\boldsymbol{\theta}\_t$, by following the steps:
   3a. Draw $\boldsymbol{\theta}\_t^+ \sim N(\boldsymbol{0}, \boldsymbol{Q})$, for $t=1,\dots, T$.
   3b. Obtain recursively $\boldsymbol{\alpha}\_{t+1}^+ = \boldsymbol{T} \boldsymbol{\alpha}\_t^+ + \boldsymbol{\eta}\_t^+$, and $\boldsymbol{\theta}\_t^+ = \boldsymbol{Z} \boldsymbol{\alpha}\_t^+$, for $t=1,\dots, T$.
4. Use $\boldsymbol{\theta}\_t^+$ to generate $\boldsymbol{y}\_t^+ \sim g(\boldsymbol{y}\_t|\boldsymbol{\theta}\_t^+)$, i.e. the approximate linear Gaussian model for $\boldsymbol{y}\_t$, by following the steps:
   4a. Draw $\boldsymbol{\varepsilon}\_t^+ \sim N(\boldsymbol{0}, \boldsymbol{A}\_t)$, for $t=1,\dots,T$, with $\boldsymbol{A}\_t$ evaluated at the mode $\hat{\boldsymbol{\theta}}\_t$.
   4b. Obtain recursively $\boldsymbol{y}\_t^+ = \boldsymbol{\theta}\_t^+ + \boldsymbol{\varepsilon}\_t^+$, for $t=1,\dots,T$.
